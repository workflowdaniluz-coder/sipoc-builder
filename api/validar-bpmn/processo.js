/**
 * GET  /api/validar-bpmn/processo?token=<t>  → carrega dados do processo para validação
 * POST /api/validar-bpmn/processo             → submete aprovação ou rejeição
  ...
  if (req.method === 'POST') {
    ...
    if (projectId) {
      await supabase.from('notifications').insert({
        ...
        body: {
          processo: sipocAtualizado.nome_processo, acao,
          validado_por: validado_por.trim(), comentario: comentario?.trim() ?? null,
    ...
    return res.status(200).json({
      ok: true,
      mensagem: acao === 'aprovar' ? 'Diagrama aprovado com sucesso.' : 'Rejeição registrada com sucesso.',
    })
  }
